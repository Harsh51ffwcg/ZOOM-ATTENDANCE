...
// --------------------------------------------------
// READ OFFLINE ATTENDANCE
// FROM GOOGLE_OFFLINE_SHEET_ID
// --------------------------------------------------
  ...
  try {
    ...
    return rows
      .filter((row: any[]) => {
        ...
        const mode = String(row[6] || "");

        return (
          date === today &&
          rowBatch.toLowerCase() ===
            batch.toLowerCase() &&
...
export async function GET(request: Request) {
  try {
    ...
    const attendance = students.map(
      (student: any) => {
        // Find offline check-in by Student ID
